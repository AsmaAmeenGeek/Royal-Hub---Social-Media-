...
// Corrected the id to 'profile-upload'
...
/*document.querySelector('#profile-upload').addEventListener('change', () => {
    const profileImages = document.querySelectorAll('.profile-popup-box img');

    // Assuming you have only one image inside .profile-popup-box
    profileImages[0].src = URL.createObjectURL(document.querySelector('#profile-upload').files[0]);

    // If you have multiple images inside .profile-popup-box (you might need to adjust this)
